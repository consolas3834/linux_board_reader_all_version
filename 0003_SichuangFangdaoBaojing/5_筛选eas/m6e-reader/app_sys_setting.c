...
struct SYS_SETTINGS {
	...
	char filter[STRING_LEN_64];
	char id[STRING_LEN_64];
	int region;
	int readpower;
	int writepower;
	int blf;
...
int sys_config_init(void)
{
	...
	sys_cfg_inset_string("system.filter", &sysConfigs.filter,STRING_LEN_64);
	sys_cfg_inset_string("system.id", &sysConfigs.id,STRING_LEN_64);
	...
	sys_cfg_inset_int("system.length", &sysConfigs.length);
	sys_cfg_inset_int("system.user_addr", &sysConfigs.user_addr);
	sys_cfg_inset_int("system.user_length", &sysConfigs.user_length);
	sys_cfg_inset_int("system.isUser", &sysConfigs.isUser);
	sys_cfg_inset_int("system.isFilter", &sysConfigs.isFilter);
	sys_cfg_inset_string("system.user_data", &sysConfigs.user_data,STRING_LEN_64);
    return 0;
}
...
void sys_config_load(int reset)
{
	...
	strcpy(sysConfigs.filter, "34");
	strcpy(sysConfigs.id, "34");
	sysConfigs.region = 1;
	sysConfigs.readpower = 500;
	...
	sysConfigs.target = 0;
	sysConfigs.length = 0;
	sysConfigs.user_addr = 0;
	sysConfigs.user_length = 0;
	sysConfigs.isUser = 0;
	sysConfigs.isFilter = 0;
	strcpy(sysConfigs.user_data, "34");
...
int sysSettingGetString(const char* name, char* value, int valueLen, int searchFlag)
{
   ...
   if (!strcmp(name, "ip"))
		...
		strncpy(value, sysConfigs.filter, valueLen);
	else if(!strcmp(name, "id"))
		strncpy(value, sysConfigs.id, valueLen);
	else if(!strcmp(name, "user_data"))
		strncpy(value, sysConfigs.user_data, valueLen);
    else
...
int sysSettingGetInt(const char* name, int* value, int searchFlag)
{
    ...
    if (!strcmp(name, "nettype"))
		...
		*value = sysConfigs.readpower;
	else if(!strcmp(name, "epc_length"))
		*value = sysConfigs.length;
	else if(!strcmp(name, "isUser"))
		*value = sysConfigs.isUser;
	else if(!strcmp(name, "isFilter"))
		*value = sysConfigs.isFilter;
	else if(!strcmp(name, "user_addr"))
		*value = sysConfigs.user_addr;
	else if(!strcmp(name, "user_length"))
		*value = sysConfigs.user_length;
    else
        printf("undefined parameter %s\n", name);
    ...
    if (!strcmp(name, "ip")) {
       ...
       if(strcmp(sysConfigs.dns, value)) {
    	...
    	if(strcmp(sysConfigs.wifiname, value)) {
    		...
    		if(strcmp(sysConfigs.filter, value)) {
			strncpy(sysConfigs.filter, value, STRING_LEN_64);
		}    	
    	} else if(!strcmp(name, "id")) {
    		if(strcmp(sysConfigs.id, value)) {
			strncpy(sysConfigs.id, value, STRING_LEN_64);
		} 
    	} else if(!strcmp(name, "user_data")) {
    		if(strcmp(sysConfigs.user_data, value)) {
			strncpy(sysConfigs.user_data, value, STRING_LEN_64);
		} 
    } else if (!strcmp(name, "wifiwd")){
...
int sysSettingSetInt(const char* name, const int value)
{
    ...
    if (!strcmp(name, "nettype")) {
		...
		if(sysConfigs.length != value)
			sysConfigs.length = value;
	} else if(!strcmp(name, "isUser")) {
		if(sysConfigs.isUser != value)
			sysConfigs.isUser = value;
	} else if(!strcmp(name, "isFilter")) {
		if(sysConfigs.isFilter != value)
			sysConfigs.isFilter = value;
	} else if(!strcmp(name, "user_addr")) {
		if(sysConfigs.user_addr != value)
			sysConfigs.user_addr = value;
	} else if(!strcmp(name, "user_length")) {
		if(sysConfigs.user_length != value)
			sysConfigs.user_length = value;
    } else {
        printf("undefined parameter %s: %d\n", name, value);
        mid_mutex_unlock(g_mutex);
        return -1;
...
int sys_readflags_get(int *flags)
{
    if(mid_mutex_lock(g_mutex)){
        printf("mutex lock error. Get doubleStack.\n");
        return -1;
...
int sys_readflags_set(int flags)
{
    if(mid_mutex_lock(g_mutex)){
        printf("mutex lock error. Get doubleStack.\n");
        return -1;
