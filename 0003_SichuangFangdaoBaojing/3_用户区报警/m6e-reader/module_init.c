...
callback(TMR_Reader *reader, const TMR_TagReadData *t, void *cookie)
{
	...
	if (0 < t->data.len)
	{
		...
		printf("  data(%d): %s\n", t->data.len, dataStr);
		sysSettingGetString("user_data", filterdata, 64, 1);
		printf("filterdata : %s, %d\n", filterdata, strlen(filterdata));
		if(!strncmp(filterdata, dataStr, strlen(filterdata))) 
		{
			int isBuzzer = 0;
...
void module_startReading()
{
	...
	int isStart,ret, i, readLen, isUser, user_addr, user_length;
	uint8_t *antennaList= NULL;
	int readpower = 500;
	...
	while(1) {
		...
		//if(isStart) {
			isUser = 0;
			sysSettingGetInt("isUser", &isUser, 1);
			//module_ant_power();
			if(isUser) {
				sysSettingGetInt("user_addr", &user_addr, 1);
				sysSettingGetInt("user_length", &user_length, 1);
		       	ret = TMR_TagOp_init_GEN2_ReadData(&op, TMR_GEN2_BANK_USER, user_addr, user_length);
      				checkerr(rp, ret, 1, "creating tagop: GEN2 read data");
      				ret = TMR_RP_set_tagop(&plan, &op);
      				checkerr(rp, ret, 1, "setting tagop");
			} else {
