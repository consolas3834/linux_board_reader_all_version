...
int module_init()
{
	...
	if(ret != 0)
		goto Err;
	printf("TMR_create %s success\n", DEVICE_NAME);

	ret = TMR_connect(rp);
	checkerr(rp, ret, 1, "connecting reader");
	if(ret != 0)
		goto Err;
	printf("TMR_connect success\n");
...
{
	char epcStr[128];
	char *epcStrold = (char *)malloc(64);
	int ret = -1, count = -1, operation;
	TMR_bytesToHex(t->tag.epc, t->tag.epcByteCount, epcStr);

	sysSettingGetInt("count1", &count, 1);
	sysSettingGetInt("operation", &operation, 1);
	sysSettingGetString("epc", epcStrold , 64, 1);
	//int flags = 0;
	//sysSettingGetInt("flags", &flags, 1);
	printf("%s, epcStrold : %s\n", epcStr, epcStrold);

	if(count > 0) {
		if(0 ==operation ) {
				sysSettingSetString("epc", epcStr);
				socket_send_begin();
				//setTimer(0, 10*1000);
				//socket_send_end();
		} else {
			//if(!strcmp(epcStr, epcStrold)) {
			socket_send_end();
			setTimer(0, 10*1000);
			//}
		}
		...
		if((0 == operation) || strcmp(epcStr, epcStrold)) {
			sysSettingSetString("epc", epcStr);
			//sys_config_save();
			socket_send_begin();
			setTimer(0, 5*1000);
